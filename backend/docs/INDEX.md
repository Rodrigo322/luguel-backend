# Documentacao do Projeto Luguel Backend

Este diretorio concentra a documentacao completa do backend.

## Indice

- [Visao Geral](./PROJECT_STATUS.md)
- [Arquitetura DDD](./ARCHITECTURE.md)
- [Setup e Execucao](./SETUP_AND_RUN.md)
- [Deploy na Vercel](./VERCEL_DEPLOY.md)
- [Referencia de API](./API_REFERENCE.md)
- [Seguranca](./SECURITY.md)
- [Testes](./TESTS.md)
- [Guia para Frontend Mobile](./MOBILE_INTEGRATION.md)

## Swagger

- UI: `/docs`
- OpenAPI JSON: `/docs/json`

## Observacao

Esta documentacao reflete o estado atual do codigo na branch `master`.
